from solvers.predicter import StreamrakPredicter
...
if __name__ == '__main__':
    filename_model = 'test_streamrak'
    filename_store_pred = 'pred_test_streamrak'

    test_data, _ = generate_toy_data(seed=3)

    streamrak_predicter = StreamrakPredicter(config_solver)
    streamrak_predicter.add_existing_model(model_name=filename_model)
    streamrak_predicter.predict(test_data)
    streamrak_predicter.save(filename_store_pred)

    yts_pred = streamrak_predicter.get_pred()
